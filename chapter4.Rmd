...
```{r readdata 20,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
library(MASS)
data("Boston")
dim(Boston)
str(Boston)
...
According to the information the Boston data frame has 506 rows and 14 columns. That is equal to my check. There is no missing valuse either. 
```{r readdata 21,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
sum(is.na(Boston))
mean(is.na(Boston))
```
...
```{r readdata 19,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
library(GGally)
library(ggplot2)
...
When summary of scaled data is compared to the original data, it is easy to see that magnitudes of min and max values have decreased. For instance, max of lstat was 37.97 and after scaling 3.54, respectively black max 396.9 and 0.44. Zn min was 0, now -0.49, rm min was 3.56, now -3.88.

Categorial variable crim is created:
```{r readdata 22,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
...
```{r readdata 23,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
...
```{r readdata 24,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
library(MASS)
data("Boston")
summary(Boston)
...
```{r readdata 25,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
...
```
Only 45 % total variance in  data set that is explained by the clusterin, which is not impressive. I checked how many clusters I should have to gain 80 % of total variance and it was 20. It is clear that it is too much, so I still keep 3 clusters. In real-life I should continue finding the correct number of clusters by other methods, e.g. silhouette.


```{r readdata 26,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
str(km)
km
```
The plot picture shows the groups are not equal size, one is 106, other 236 and the third 164. There are some pairs, in which the grouping seems to work nicely; for instance medvd & lstat, stat & nox,dis & nox and rm & nox. But for some pairs the groups are unclear, for instance rm&ptratio, age & ptratio, ptratio & lstat. 

```{r fig24, fig.path="figures/",fig.dim=c(12,12)}
...
```{r readdata 27,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
library(MASS)
library(ggplot2)
library(tidyr)
library(cluster)
...
```{r readdata 28,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
...
```{r readdata 29,echo=TRUE,results='hide',message=FALSE,warning=FALSE}
